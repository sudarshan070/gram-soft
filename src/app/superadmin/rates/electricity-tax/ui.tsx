...
export function SuperAdminElectricityTaxSlabsClient(props: { rates: SlabRow[] }) {
  ...
  async function createRate(values: {
    ...
    try {
      ...
      const res = await fetch("/api/global-rates/electricity-tax-slabs", {
        method: "POST",
  ...
  async function saveEdit(values: {
    ...
    try {
      ...
      const res = await fetch(`/api/global-rates/electricity-tax-slabs/${editing._id}`, {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(payload),
      });
  ...
  function confirmDelete(row: SlabRow) {
    modal.confirm({
      ...
      cancelText: "रद्द",
      async onOk() {
        const res = await fetch(`/api/global-rates/electricity-tax-slabs/${row._id}`, {
          method: "DELETE",
        });
