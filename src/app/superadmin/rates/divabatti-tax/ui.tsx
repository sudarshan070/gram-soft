...
export function SuperAdminDivabattiTaxSlabsClient(props: { rates: SlabRow[] }) {
  ...
  async function createRate(values: {
    ...
    try {
      const payload = {
        ...
        slabToSqFt:
          values.slabTo == null || values.slabTo === ("" as unknown as number)
            ? null
            : toStoredSqFt(unit, values.slabTo),
        rate: values.rate,
        effectiveFrom: values.effectiveFrom,
      };

      const res = await fetch("/api/global-rates/divabatti-tax-slabs", {
        method: "POST",
  ...
  async function saveEdit(values: {
    ...
    try {
      ...
      const res = await fetch(`/api/global-rates/divabatti-tax-slabs/${editing._id}`, {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(payload),
      });
  ...
  function confirmDelete(row: SlabRow) {
    modal.confirm({
      ...
      cancelText: "रद्द",
      async onOk() {
        const res = await fetch(`/api/global-rates/divabatti-tax-slabs/${row._id}`, {
          method: "DELETE",
        });
